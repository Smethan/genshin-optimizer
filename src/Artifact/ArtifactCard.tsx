import { faBan, faBriefcase, faChartLine, faEdit, faInfoCircle, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { Lock, LockOpen } from '@mui/icons-material';
import { Box, Button, ButtonGroup, CardActions, CardContent, CardMedia, Chip, Divider, Grid, IconButton, ListItemIcon, MenuItem, Tooltip, Typography } from '@mui/material';
...
export default function ArtifactCard({ artifactId, artifactObj, onEdit, onDelete, mainStatAssumptionLevel = 0, effFilter = allSubstatFilter }: Data): JSX.Element | null {
  ...
  return <CardLight sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
    ...
    <CardActions>
      <Grid container sx={{ flexWrap: "nowrap" }}>
        {editable ? <Grid item xs="auto" flexShrink={1}>
          <DropdownButton
            title={characterSheet?.name ?? t`filterLocation.inventory`}
            id={`charsel-${id}`}
            size="small"
            color={location ? "success" : "primary"}
            startIcon={characterSheet?.thumbImg ? <img src={characterSheet.thumbImg} className="inline-icon" alt="" /> : <FontAwesomeIcon icon={faBriefcase} />}>
            <MenuItem onClick={() => equipOnChar("")} selected={location === ""} disabled={location === ""}>
              <ListItemIcon>
                <FontAwesomeIcon icon={faBriefcase} />
              </ListItemIcon>
              <Typography variant="inherit" noWrap>
                {t`filterLocation.inventory`}
              </Typography>
            </MenuItem>
            <Divider key="div" />
            {!!characterSheets && CharacterMenuItemArray(characterSheets, database._getCharKeys().sort(), equipOnChar, location)}
          </DropdownButton>
        </Grid> : <Grid item xs="auto"><b>{locationName}</b></Grid>}
        <Grid item flexGrow={1} sx={{ mr: 1 }} />
        {editable && <Grid item xs="auto">
          <ButtonGroup sx={{ height: "100%" }}>
            {!!onEdit && <Button color="info" onClick={onEdit} size="small">
            ...
            {!!onDelete && <Button color="error" size="small" onClick={onDelete}>
